...
public class AirportsAcceptValidator extends AbstractValidator<Flight> {
    private static final Properties prohibitedAirportCodes = PropertiesLoader.loadPropertiesFromFile("prohibitedAirports.properties");

    @Override
    public Errors validate(Flight flight, Errors errors) {

        if (prohibitedAirportCodes.containsKey(flight.getDeparture().getIataCode())) {
            errors.addError(new ObjectError("Flying to Departure Airport " +
                    flight.getDeparture().getName() + "prohibited by " +
        ...
        if (prohibitedAirportCodes.containsKey(flight.getDestination().getIataCode())) {
            errors.addError(new ObjectError("Flying to Destination Airport " +
                    flight.getDestination().getName() + "prohibited by " +
